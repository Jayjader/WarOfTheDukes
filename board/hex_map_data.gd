...
func paths_for(unit: GamePiece, others: Array) -> Dictionary:
	var frontier = [[unit.tile, 0]]
	var reached = {unit.tile: [unit.tile, 0]}
	...
	while len(frontier) > 0:
		...
		for direction in Util.cube_directions:
			...
			if enemies.any(func(enemy): return enemy.tile == to_):
				...
				continue
			var from_neighbours = Util.neighbours_to_tile(next_axial)
			var to_neighbours = Util.neighbours_to_tile(to_)
			if enemies.any(func(enemy): return enemy.tile in from_neighbours) and enemies.any(func(enemy): return enemy.tile in to_neighbours):
